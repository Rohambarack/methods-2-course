...
using the derivative test you  find the extreme value as -1/10

- When having found an extremum how can you be sure it is a min or max? (think higher order derivatives.) 

using the second derivative test you find f''(x) is positive, so it is a local minimum
...
```{r}
xme2 <- seq(0,24, by=0.05)
plot(xme2,sin_function(xme2), type ="l")
...
xme = 0
repeat{
  print(optim(xme, sin_function, method = "CG")$par)
  xme = xme +1
  if (xme > 24) break("X limit reached")
}
```
  
  c) Check if the they are indeed minimums using the second derivative rule we learned last class. 
  
  4.712389 , 10.99557 , 17.27876 , 23.56195
  
the second derivative of sin(x) is sin(x) and these values are all positive, so they are minimums.
...
```{r}
optimize(function(x){sin(x)},interval = c(0,24),maximum = T)
```
```{r}
xme3 = 0
repeat{
 if (
    (
     round(optim(xme3, sin_function, method = "CG",control = list(fnscale = -1))$par, digits = 2)
     
     !=
     
     (
     round(optim(xme3+1, sin_function, method = "CG",control = list(fnscale = -1))$par,
     digits = 2)
     )
     
 {print(optim(xme3, sin_function, method = "CG",control = list(fnscale = -1))$par)}
   
  xme3 = xme3 +1
  if (xme3 > 24) break("X limit reached")
}
```
