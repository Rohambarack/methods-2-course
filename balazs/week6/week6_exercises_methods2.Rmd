...
```{r}
set.seed(1666)
mydesignmtrx <- matrix(sample(30:70,10), ncol = 1, nrow = 10)


mydesignmtrx <- cbind(mydesignmtrx,rep(1,10))
mydesignmtrx[,c(1,2)] <- mydesignmtrx[,c(2,1)]
...
```{r}
myy <- mydesignmtrx%*%beta
```
    
        3.1) Plot the relationship between y ~ x
```{r}
plot(x=mydesignmtrx[,2],y=myy)
```
        
        3.2) Does something look odd?
        answer: it seems rather linear.
...
```{r}
set.seed(1666)
errors <- c(sample(-10:25, 10))
yhat <- myy + errors

plot(x=mydesignmtrx[,2],y=yhat)
...
beta1 <- (solve(t(mydesignmtrx[,2])%*%mydesignmtrx[,2]))%*%(t(mydesignmtrx[,2])%*%yhat)
...
```
        ...
        answer: It did not match exactly. I would guess, because yhat has the added error values
        
        5.2) run normal lm() on yhat ~ x. Does the beta coefficients match? 
        answer: They don't match
```{r}
lm(yhat~mydesignmtrx[,2])
```
