...
```{r}
DF_1_ALL_Data <- read_csv("C:/github/methods-2-course/Data for the book/KidIQ/data/kidiq.csv")
```
...
```{r}
kid_bayes_mod <- stan_glm(kid_score ~ mom_iq,data = DF_1_ALL_Data)
```


```{r}
kidbayes_post_pred <- posterior_predict(kid_bayes_mod, newdata = data.frame(mom_iq = 100))
hist(kidbayes_post_pred)
```
